 ...
 * Connects to self-hosted Ollama instance via Open WebUI
 */
...
export async function callOllama(prompt, options = {}) {
  try {
    ...
    console.log('🤖 Calling Ollama API (Open WebUI)...');
    console.log('   Base URL:', OLLAMA_CONFIG.baseUrl);
    console.log('   Model:', model);

    // Open WebUI Ollama API format
    const requestBody = {
      model: model,
      prompt: prompt,
      stream: false,
    ...
    // Open WebUI requires Authorization header
    const headers = {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${OLLAMA_CONFIG.apiKey}`
    };

    const response = await fetch(`${OLLAMA_CONFIG.baseUrl}/ollama/api/generate`, {
      method: 'POST',
      headers: headers,
